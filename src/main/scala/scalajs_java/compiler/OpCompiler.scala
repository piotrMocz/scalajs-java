...
object OpCompiler {
  ...
  def compileBinopCode(op: Tag, tpe: Type): BinaryOp.Code = tpe match {
    case JExprType(jtype) if jtype.isPrimitive => jtype.getTag match {
        ...
        case _ =>
          throw new Exception(s"Not a primitive type: ${jtype.getTag}")
      }
    ...
    case JExprType(jtype) =>
      if (jtype.getTag == TypeTag.CLASS &&
        ...
        throw new Exception(s"Cannot compile type: ${jtype.getTag}")
